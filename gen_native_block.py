...
than silently mis-sizing a member:
  * MapProperty when no --reference-header declaration is available.  For existing native
    members, --reference-header reuses the exact UHT declaration instead of guessing K/V types.
...
Usage:
    python gen_native_block.py <manifest.json> <package.u> <OwnerName> [--actors Manifest.txt]
        [--reference-header EngineGameEngineClasses.h]
"""
import json
import re
import struct
...
def reference_declarations(header_path, owner):
    """Return exact existing native declarations keyed by member name for one UHT prop block.

    This is intentionally conservative: it only reads declarations between UE3's generated
    BEGIN/END PROPS markers and never invents a TMap signature. It lets package metadata
    determine ordering while preserving a known-good native declaration for serialization
    forms (notably MapProperty / Map_Mirror) whose key/value types are not encoded by the simple
    trailing-reference rule used for the other UProperty classes.
    """
    if not header_path:
        return {}
    text = open(header_path, encoding="latin1", errors="replace").read()
    begin_marker = "//## BEGIN PROPS %s" % owner
    end_marker = "//## END PROPS %s" % owner
    begin = text.find(begin_marker)
    end = text.find(end_marker, begin + len(begin_marker)) if begin >= 0 else -1
    if begin < 0 or end < 0:
        raise ValueError("could not find generated property block for %s in %s" %
                         (owner, header_path))

    out = {}
    # UHT declarations in this block are one-line declarations. Capture the final identifier
    # before an optional static-array suffix / bitfield width and semicolon. Access specifiers
    # and preprocessor lines deliberately do not match.
    member_re = re.compile(r"\b([A-Za-z_]\w*)\s*(?:\[[^]]+\])?\s*(?::\s*\d+)?\s*;\s*$")
    for raw in text[begin + len(begin_marker):end].splitlines():
        stripped = raw.strip()
        m = member_re.search(stripped)
        if m:
            out[m.group(1)] = stripped
    return out
...
def generate(manifest, package, owner, manifest_txt=None, reference_header=None):
    exports, imports, blob = load(manifest, package)
    actors = actor_classes(manifest_txt) if manifest_txt else set()
    reference = reference_declarations(reference_header, owner) if reference_header else {}
    ...
    for e in members:
        ...
        # MapProperty serializes key/value type information that the simple trailing-reference
        # rule cannot recover safely. Map_Mirror is the older UE3 native-TMap mirror form.
        # If this member already exists, retain UHT's exact declaration from a known-good header;
        # Judgment-only maps still fail loudly rather than receiving a guessed ABI.
        if cls == "MapProperty" or (cls == "StructProperty" and target == "Map_Mirror"):
            if name in reference:
                lines.append("    %s" % reference[name])
            else:
                lines.append("    %s /* %s */ %s;" % (UNSUPPORTED, cls, name))
            continue
...
if __name__ == "__main__":
    ...
    if "--actors" in args:
        ...
        del args[i:i + 2]
    ref_header = None
    if "--reference-header" in args:
        i = args.index("--reference-header")
        ref_header = args[i + 1]
        del args[i:i + 2]
    if len(args) != 3:
        raise SystemExit("usage: gen_native_block.py <manifest.json> <package.u> <OwnerName> "
                         "[--actors Manifest.txt] [--reference-header header.h]")
    for line in generate(args[0], args[1], args[2], mtxt, ref_header):
        print(line)
